...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self,
        ...
        # batch index
        # 将数据划分到一个个batch中
        bi = np.floor(np.arange(n) / batch_size).astype(np.int)#对输入的多维数组逐元素进行向下取整.
        # 记录数据集划分后的总batch数
...
def load_image(self, index):
    ...
    img = self.imgs[index]
    if img is None:  # not cached->当前图像无缓存
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        ...
        if i == 0:  # top left
            # 创建马赛克图像
            img4 = np.full((s * 2, s * 2, img.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles,用114进行填充
...
def random_affine(img, targets=(), degrees=10, translate=.1, scale=.1, shear=10, border=0):
    ...
    if n:
        ...
        # [4*n, 3] -> [n, 8]
        xy = (xy @ M.T)[:, :2].reshape(n, 8)#对每个点进行转置变换
